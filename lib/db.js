...
function createBand(name, email, folderId, accountingSpreadsheetId, invoicesFolderId) {
    return new Promise((resolve, reject) => {
        const db = getDatabase();

        db.run(
            'INSERT INTO bands (name, email, folder_id, accounting_spreadsheet_id, invoices_folder_id) VALUES (?, ?, ?, ?, ?)',
            [name, email, folderId, accountingSpreadsheetId, invoicesFolderId],
            function(err) {
                db.close();
                if (err) {
                    return reject(err);
