...
async function errorHandler(err, req, res, next) {
    ...
    // Check for expired/invalid Google OAuth token
    if (googleAuth.isInvalidGrantError(err)) {
        console.log('Detected expired Google OAuth token - clearing from database');
        await googleAuth.clearRefreshToken();

        // Set user-friendly error message
        req.flash.error('Your Google authentication has expired. Please authenticate with Google again.');

        // Redirect back to the page where the error occurred (referer), or fallback to /bands
        const redirectTo = req.headers.referer || '/bands';
