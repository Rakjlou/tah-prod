...
/**
 * Get all Qonto transactions with their link information for admin panel
 * @returns {Promise<array>} - Array of Qonto transactions with enriched link data
 */
const getAllQontoTransactionsWithLinks = async () => {
    // Get all Qonto transactions
    const qontoTransactions = await getAll(
        'SELECT * FROM qonto_transactions ORDER BY settled_at DESC'
    );

    // Get all links
    const allLinks = await getAll(
        `SELECT qtl.qonto_id, qtl.transaction_id, qtl.allocated_amount, qtl.qonto_amount
         FROM qonto_transaction_links qtl`
    );

    // Create a map of qonto_id to link information
    const linkMap = {};
    allLinks.forEach(link => {
        if (!linkMap[link.qonto_id]) {
            linkMap[link.qonto_id] = {
                linkedTransactionIds: [],
                totalAllocated: 0
            };
        }
        linkMap[link.qonto_id].linkedTransactionIds.push(link.transaction_id);
        linkMap[link.qonto_id].totalAllocated += Math.abs(link.allocated_amount || link.qonto_amount || 0);
    });

    // Enrich each Qonto transaction with link information
    const enrichedTransactions = qontoTransactions.map(tx => {
        const linkInfo = linkMap[tx.qonto_id] || { linkedTransactionIds: [], totalAllocated: 0 };
        const available = Math.abs(tx.amount) - linkInfo.totalAllocated;

        return {
            ...tx,
            isLinked: linkInfo.linkedTransactionIds.length > 0,
            linkedTransactionIds: linkInfo.linkedTransactionIds,
            totalAllocated: linkInfo.totalAllocated,
            available: available,
            isFullyAllocated: available < 0.01
        };
    });
...
module.exports = {
    ...
    getAllQontoTransactionsWithLinks
};
